...
class SileroVAD:
    ...
    def __init__(
        ...
        Args:
            model_path: VAD 模型路径
            sample_rate: 音频采样率
            min_silence_duration: 最小静音时长（秒），触发分段
            min_speech_duration: 最小语音时长（秒），过滤短语音
    ...
    def _create_vad(self):
        ...
        # [优化] window_size 控制VAD的窗口大小
        # 注意：Silero VAD 模型要求 window_size 必须是 512（模型输入维度固定）
        # 不能修改为其他值，否则会报错 "Got invalid dimensions"
